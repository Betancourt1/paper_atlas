...
- Mobile, accessibility, and motion behavior: At widths up to 640 px, rebuild only the two Figure 1 branch composites from the inspected 3200 × 1200 source using these safe bounds, which replace every revision-15 coordinate. Each source row receives 32 px neutral canvas padding on all four sides before vertical assembly: question row x=0..2125, y=0..165 (2126 × 166 px), including the complete outer question stroke through x≈2093 plus right-side source whitespace; shared Search/Open row x=560..1500, y=145..550 (941 × 406 px), including both complete rounded boxes, their connecting arrow, and source whitespace beyond the right stroke at x≈1461; success row x=0..880, y=520..1175 (881 × 656 px), including the complete fork input, three orange boxes, arrows, and check marks; failure row x=1150..2130, y=520..1175 (981 × 656 px), including the complete fork input, three purple boxes, arrows, and both crosses through at least x=2090 plus right-side source whitespace. Center each independently padded row in its branch composite; do not use a fixed source canvas that creates sampled blank fragments. Keep the complete trajectory plot as block three using x=2320..3199, y=130..1085 (880 × 956 px) plus the same 32 px neutral padding. Modification record: crop, neutral padding, duplication of the complete question/shared prefix, centering, and vertical assembly only; source pixels remain unscaled and unaltered, and no stroke may touch a composite edge. No redraw, relabeling, interpolation, clipping, mixed branch fragment, or source-derived blank strip. Retain block-specific alt text, max-width: 100%, height: auto, and no motion or scrollbar.
...
- Selection rationale: Treatment A remains evidence-correct, but revision 16 requires the safe inspected bounds and neutral padding specified above; reject the revision-15 coordinates because they clip the question, shared prefix, and failure-branch strokes.

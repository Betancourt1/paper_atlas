...
- Mobile, accessibility, and motion behavior: At widths up to 640 px, use exactly three intact source-coordinate strips from the 3200 × 1200 original Figure 1; do not recompose or independently center internal rows. Strip 1, question plus shared prefix: x=0..2130, y=0..550 (2131 × 551 px), retaining the complete question box, its vertical connector, the complete Search and Open boxes, and the connector down to the fork approach in their original coordinates. Strip 2, complete branching region: x=0..2130, y=540..1179 (2131 × 640 px), retaining the entire horizontal fork connector and both complete success and failure branches, including every box, arrow, check, and cross in their original relative positions; the 11 px overlap with Strip 1 preserves connector continuity. Strip 3, complete trajectory plot: x=2300..3199, y=100..1100 (900 × 1001 px), retaining the full axes, points, curves, labels, and annotations. Add 32 px neutral canvas margin to every side of each derived strip only after cropping; do not move, resize, duplicate, or reassemble any source element within a strip. Render each padded strip as a separate full-width block with `max-width: 100%`, `height: auto`, and CSS containment that produces no internal or page scrollbar; verify labels and connector relationships remain readable at 390 px. Modification record: three source-coordinate crops plus neutral outer margin only; source pixels and spatial relationships remain unscaled and unaltered. No redraw, relabeling, interpolation, clipping, or blank internal fragment. Retain strip-specific alt text and no motion.
...
- Selection rationale: Treatment A remains evidence-correct, but revision 17 requires three intact source-coordinate strips; reject revision 16 because independently centered rows disconnect the shared prefix from the original fork geometry.
...
- Evidence deviations: `REVISION_16_REJECTED` — replace the disconnected branch composites with intact source-coordinate strips; do not alter the complete desktop original.
